...
@section('title', 'Review Queue - MedFlow CRM')
@section('breadcrumb', 'Review Queue')
...
<div class="page-header animate-in">
  <div>
    ...
    <p class="page-subtitle">Escalations, consent, and today's consultations &bull; {{ now()->format('l, d F Y') }}</p>
  </div>
</div>
...
<div class="card animate-in" style="animation-delay:.04s;border:1px solid #fca5a5;margin-bottom:18px;padding:0;">
  <div style="padding:14px 20px;border-bottom:1px solid #fca5a5;display:flex;align-items:center;justify-content:space-between;">
    <div style="display:flex;align-items:center;gap:10px;">
      ...
      <div class="card-title" style="color:var(--danger);">Escalations — Doctor Review Required</div>
    </div>
          ...
          <td>
            <div style="display:flex;gap:6px;">
              ...
              <form method="POST" action="{{ route('appointments.status', $appt) }}">
                ...
                <button type="submit" class="btn btn-primary btn-sm" style="font-size:.73rem;">Clear &amp; Continue</button>
              </form>
            </div>
          </td>
        </tr>
...
<div class="card animate-in" style="animation-delay:.04s;background:var(--success-light);border:1px solid #6ee7b7;margin-bottom:18px;">
  <div style="display:flex;align-items:center;gap:10px;color:#065f46;">
    ...
    <span style="font-weight:500;">No escalations — all clear!</span>
  </div>
</div>
@endif
...
<div class="grid-2-1 animate-in" style="animation-delay:.08s;align-items:start;">
  ...
  <div class="card" style="padding:0;">
    ...
    <div class="empty-state" style="padding:24px;"><p>No consultations assigned today</p></div>
    @else
    <div class="table-wrap">
      <table>
  ...
  <div style="display:flex;flex-direction:column;gap:16px;">
    ...
    <div class="card" style="{{ $consentPending->isNotEmpty() ? 'border:1px solid #fde68a;' : '' }}">
      <div class="card-header">
        <div class="card-title" style="{{ $consentPending->isNotEmpty() ? 'color:var(--warning);' : '' }}">Consent Pending</div>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border-light);">
        ...
        <a href="{{ route('patients.show', $patient) }}" class="btn btn-secondary btn-sm" style="font-size:.72rem;">View</a>
      </div>
      @empty
      <p style="color:var(--text-tertiary);font-size:.83rem;padding:8px 0;">All patients have given consent</p>
      @endforelse
    </div>

    {{-- Quick Reference --}}
    <div class="card">
      <div class="card-title" style="margin-bottom:12px;">Clinical Guidelines</div>
      <div style="font-size:.8rem;color:var(--text-secondary);display:flex;flex-direction:column;gap:8px;">
        <div style="display:flex;align-items:flex-start;gap:8px;">
          ...
          <span>Do not proceed if patient is pregnant, has pacemaker, or active metal implants in treatment area</span>
        </div>
        <div style="display:flex;align-items:flex-start;gap:8px;">
          <span style="color:var(--danger);font-weight:700;flex-shrink:0;">✕</span>
          <span>Do not proceed without signed consent form</span>
        </div>
        <div style="display:flex;align-items:flex-start;gap:8px;">
          <span style="color:var(--warning);font-weight:700;flex-shrink:0;">!</span>
          <span>Fitzpatrick V–VI requires reduced energy settings — confirm before laser</span>
        </div>
        <div style="display:flex;align-items:flex-start;gap:8px;">
          <span style="color:var(--warning);font-weight:700;flex-shrink:0;">!</span>
          <span>Review medication list for photosensitisers (isotretinoin, antibiotics)</span>
        </div>
      </div>
    </div>

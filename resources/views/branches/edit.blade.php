...
@section('title', 'Edit Branch - MedFlow CRM')
@section('breadcrumb', 'Branches / Edit')
...
<div class="grid-2-1 animate-in" style="animation-delay:.05s;align-items:start;">
  ...
  <div style="display:flex;flex-direction:column;gap:18px;">
    ...
    <div class="card">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        ...
        <div style="text-align:center;padding:12px;background:var(--bg-tertiary);border-radius:var(--radius-md);">
          ...
          <div style="font-size:.72rem;color:var(--text-tertiary);">Appts</div>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-title" style="margin-bottom:14px;">Staff at This Branch</div>
      ...
      <div style="display:flex;align-items:center;gap:9px;padding:7px 0;border-bottom:1px solid var(--border-light);">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div style="font-size:.72rem;color:var(--text-tertiary);">{{ ucfirst(str_replace('_',' ',$member->employee_type ?? '')) }}</div>
        </div>
        <span class="badge {{ $member->employment_status === 'active' ? 'badge-green' : 'badge-gray' }} badge" style="font-size:.68rem;">{{ ucfirst($member->employment_status) }}</span>
      </div>
      @endforeach
      @if($branch->staff->count() > 6)
      <div style="text-align:center;padding:10px 0;color:var(--text-tertiary);font-size:.8rem;">+{{ $branch->staff->count() - 6 }} more staff</div>
      @endif
    </div>
    @endif
    ...
    <div class="card">
      <div class="card-title" style="margin-bottom:14px;">Treatment Rooms</div>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px solid var(--border-light);">
        ...
        <span class="badge {{ $room->status === 'available' ? 'badge-green' : 'badge-gray' }}">{{ ucfirst($room->status) }}</span>
      </div>
      @endforeach
    </div>
    @endif
